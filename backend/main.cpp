...
CROW_ROUTE(app, "/api/images/")
    ...
    ([](const crow::request& req, crow::response& res) {
        ...
        // Tangani GET request
        std::string upload_folder = "uploads/";  // Pastikan folder sesuai
        ...
        if (files.empty()) {
            res.code = 404;
            res.body = "Tidak ada gambar yang ditemukan.";
            res.set_header("Access-Control-Allow-Origin", "*");
            res.end();
            return;
        }
        ...
        res.set_header("Access-Control-Allow-Origin", "*");  // Tambahkan CORS
...
// api upload gambar 
    ...
    ([](const crow::request& req, crow::response& res) {
        ...
        if (headers.find("multipart/form-data") == std::string::npos) {
            res.code = 400;
            res.body = "Format multipart tidak valid.";
            res.end();
            return;
        }
        ...
        if (boundaryPos == std::string::npos) {
            res.code = 400;
            res.body = "Boundary tidak ditemukan.";
            res.end();
            return;
        }
        ...
        if (fileEnd == std::string::npos) {
            res.code = 400;
            res.body = "Tidak dapat menemukan akhir file.";
            res.end();
            return;
        }
        ...
        std::string filename = "uploads/" + oss.str() + ".jpg";
        ...
        res.code = 200;
        res.body = "{\"message\": \"Upload berhasil!\", \"filename\": \"" + filename + "\"}";
        res.end();
    });

// Handle preflight request untuk CORS
CROW_ROUTE(app, "/api/upload").methods(crow::HTTPMethod::OPTIONS)
